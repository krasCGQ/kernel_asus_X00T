...
static int __init countrycodeCmd(char *str)
{
    if (!strcmp(str, "ww")) {
	BR_countrycodeCmd = COUNTRY_OTHER;
    } else {
	BR_countrycodeCmd = COUNTRY_BR;
    }
    return 1;
}

__setup("androidboot.board_id_st=", countrycodeCmd);
...
//Huaqin added by tangqingyong at 20180206 for USB alert start
	register_usb_alert();
//Huaqin added by tangqingyong at 20180206 for USB alert end
//Huaqin added by tangqingyong at 20180212 for usb_otg stasysfs_remove_grouprt
	register_usb_otg();
//Huaqin added by tangqingyong at 20180212 for usb_otg end
